...
const NotificationDropdown = () => {
  ...
  useEffect(() => {
    const socket = new WebSocket(`wss://${ingressDomain}/ws/notification/?token=${token}`);
    ...
    switch (type) {
      case 'follow':
        if (sender && sender.profile) {
          return <img src={sender.profile} className="h-8 w-8 mt-2 object-cover rounded-full " />;
        } else {
      ...
      case 'meeting':
        if (team && team.profile_image) {
          return <img src={team.profile_image} className="h-8 w-8 mt-2 object-cover rounded-full" />;
        } else {
