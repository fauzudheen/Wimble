...
const Login = () => {
  ...
  const onSubmit = async(data) => {
    ...
    setIsLoading(true);
    try{
      const response = await axios.post(`${GatewayUrl}api/login/`, data)
    ...
    <div className="flex justify-center items-center min-h-screen dark:bg-gray-900 p-4">
      ...
      <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md w-full max-w-md z-10 relative">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {serverErrors.non_field_errors && serverErrors.non_field_errors.map((error, index) => <p className="text-sm text-red-500 text-center my-6" key={index}>{error}</p>)}
          <button
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Not a member?{' '}
            <a onClick={() => navigate('/signup')} className="text-cyan-500 hover:text-cyan-600 dark:text-cyan-400 dark:hover:text-cyan-300 cursor-pointer">
              Join the network
            </a>
          </p>
        </div>
